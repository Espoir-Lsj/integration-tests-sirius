...
class TestAccept:
    ...
    def test_04(self,create):
        """接收临调单 ID为空"""
        response = accept.success(None, goodsId=1, quantity=1)
        assert response['msg'] == '请选择临调单'

    def test_05(self, create):
        """接收临调单 ID错误"""
        response = accept.success(0, goodsId=1, quantity=1)
    ...
    def test_07(self, create):
        """发货方式错误"""
        response = accept.success(self.adhocOrderId,goodsId=self.goodsId, quantity=1,deliveryMode='1')
        assert response['msg'] == '发货方式错误'
